...
SIGNAL_GGH_TO_GAMMAGAMMA_WEIGHT = float(
    os.environ.get(
        "SIGNAL_GGH_TO_GAMMAGAMMA_WEIGHT",
        str(SIGNAL_GGH_K_FACTOR * YR4_BR_H_TO_GAMMAGAMMA),
    )
)
...
# name | category | model | process | madspin_decay | weight_scale | response_mode
SAMPLES: list[Sample] = [
    ...
    Sample("bkg_prompt_aa", "Backgrounds", "sm", "p p > a a", "", 1.0, "genuine"),
    Sample("bkg_gamma_j", "Backgrounds", "sm", "p p > a j", "", 1.0, "gammajet"),
    Sample("bkg_dy_ee", "Backgrounds", "sm", "p p > e+ e-", "", 1.0, "dielectron"),
    ...
    # Sample("bkg_loop_gg_aa", "Backgrounds", "loop_sm", "g g > a a [noborn=QCD]", "", 1.0, "genuine"),
    # A dijet sample will need a dedicated double-fake response mode before it
    # can be enabled consistently.
]
...
class Config:
    ...
    def analysis_target(self) -> str:
        targets = {
            "ssc": "HwSimPostAnalysis_gammagamma_SSC",
            "none": "HwSimPostAnalysis_gammagamma",
        }
        try:
            return targets[self.detector_response]
        except KeyError as error:
            raise ValueError(f"unknown detector response: {self.detector_response}") from error
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--run-samples",
        default=env_text("RUN_SAMPLES", "signal_gg_h_aa,bkg_prompt_aa,bkg_gamma_j,bkg_dy_ee"),
    )
    parser.add_argument(
        "--detector-response",
        choices=DETECTOR_RESPONSES,
        default=env_text("DETECTOR_RESPONSE", "ssc"),
        help=(
            "post-analysis detector model: 'ssc' applies the GEM response, efficiencies, "
            "and weighted photon fakes; 'none' runs the legacy unsmeared HwSim photon analysis"
        ),
    )
...
def parse_config(argv: Sequence[str]) -> Config:
    ...
    # argparse does not apply ``choices`` validation to a string default read
    # from the environment, so validate that path explicitly.
    if args.detector_response not in DETECTOR_RESPONSES:
        parser.error(
            f"argument --detector-response: invalid choice: {args.detector_response!r} "
            f"(choose from {', '.join(DETECTOR_RESPONSES)})"
        )
    explicit_herwig_env = args.herwig_env.expanduser() if args.herwig_env else None
    # Site modules commonly export HERWIG_ENV as an installation directory,
    # whereas this option means a sourceable activation script.
    ...
    if args.no_herwig_module:
        herwig_module = None
    # A loaded module may export HERWIG_ENV as its installation prefix rather
    # than an activation script.  A selected module takes precedence and must
    # not also be sourced as though that prefix were a shell file.
    herwig_env = None if herwig_module is not None else explicit_herwig_env
...
def selected_samples(cfg: Config) -> list[tuple[int, Sample]]:
    return [
        (sample_index, sample)
        for sample_index, sample in enumerate(SAMPLES)
        if sample_requested(sample, cfg)
    ]


def validate_detector_response(cfg: Config, selected: Sequence[tuple[int, Sample]]) -> None:
    if cfg.detector_response != "none":
        return
    fake_samples = [sample.name for _, sample in selected if sample.response_mode != "genuine"]
    if fake_samples:
        die(
            "--detector-response none cannot model the photon fakes required by "
            f"{','.join(fake_samples)}; use --detector-response ssc or select only "
            "genuine-photon samples"
        )


def analysis_command(sample: Sample, root_input: Path, seed: int, cfg: Config) -> list[object]:
    command: list[object] = [
        cfg.analysis_exe,
        root_input,
        "-t",
    ...
    if cfg.detector_response == "ssc":
        command.extend(["--response-mode", sample.response_mode, "--seed", seed])
    return command
...
def patch_run_card(card: Path, seed: int, cfg: Config) -> None:
    ...
    if cfg.dry_run:
        ...
        print(
            "pdlabel={pdlabel}, pdlabel1={pdlabel1}, pdlabel2={pdlabel2}, "
            "lhaid={lhaid}".format(**updates)
        )
        print(
...
def run_generate_events(process_dir: Path, runtime_lib_dir: Path | None, cfg: Config) -> None:
    ...
    run_runtime_command(
        ...
        cfg,
        cwd=process_dir,
        env=generation_environment(runtime_lib_dir),
    )
...
def run_sample(index: int, sample: Sample, cfg: Config) -> None:
    ...
    print(
        ...
        f"weight_scale={sample.weight_scale} detector_response={cfg.detector_response} "
        f"response_mode={sample.response_mode} seed={seed}"
    )
    ...
    # The Herwig runtime module also supplies the LHAPDF installation used by
    # the forced NNPDF4.0 run-card settings, so MG5 generation must share it.
    run_runtime_command(mg5_command(cfg, mg5_card), cfg)
    ...
    run_runtime_command(analysis_command(sample, root_input, seed, cfg), cfg)


def main(argv: Sequence[str]) -> int:
    cfg = parse_config(argv)
    selected = selected_samples(cfg)
    if not selected:
        die(f"no samples selected by RUN_SAMPLES={cfg.run_samples}")
    validate_detector_response(cfg, selected)
    require_inputs(cfg)

    log(
        f"Building gamma-gamma post-analysis "
        f"(detector_response={cfg.detector_response}, target={cfg.analysis_target})"
    )
    run_runtime_command(["make", "-C", cfg.analysis_code_dir, cfg.analysis_target], cfg)

    for sample_index, sample in selected:
        run_sample(sample_index, sample, cfg)
